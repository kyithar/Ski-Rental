...
void base_cache::initialize(){
    ...
    //cache_size = par("C");  //cache sizez
    cache_size= rcacheini[r];r++;
...
void base_cache::store(cMessage *in){
    ...
    if (decisor->data_to_cache((ccn_data*)in ) ){
		...
		decision_yes++;
		//</aa>
		data_store((ccn_data*)in ); //store is an interface funtion: each caching node should reimplement that function
